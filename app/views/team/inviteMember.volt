...
{{ content() }}
    ...
    <section class="content-header">
      ...
      <h1>
        <a href="<?php echo $this->url->get('team/index');?>" class="btn tomboladd"><i class="fa fa-reply" aria-hidden="true"></i> back</a> List
        <small>member</small>
      </h1>
      <ol class="breadcrumb">
...
<!--                 <div class="panel-heading">Team Invitation</div>-->
                ...
                <table id="table" class="table table-hover table-mc-light-blue">
						...
						<tbody id="teampagination">
							<?php if(@$availableTalentDTOs):?>
						<?php $i = 0; ?>
						<?php foreach ($availableTalentDTOs as $row) :?>
						<tr>
							<td data-title="Company Name"> <?php echo _hinter($row)->name(); ?> </td>
					   
							<td data-title="Email"><?php echo _hinter($row)->email(); ?> </td>
							
							<td data-title="Action">
							   <a href='<?php echo $this->url->get('team/inviteTalent/'.$row->id());?>' class="btn tomboladd">Profile Info</a>
								 
		<!--			<a href='<?php // echo $this->url->get('team/sendInvitation/'.$row->id());?>' class="btn btn-default">Invite</a>-->
							</td>
						</tr>
